...
static int multipath_map(struct mpconf *conf)
{
	...
	for (i = 0; i < disks; i++) {
		struct md_rdev *rdev =
			rcu_dereference(conf->multipaths[i].rdev);
	...
	for (;;) {
		...
		if ((mp_bh->path = multipath_map(conf)) < 0) {
			...
			*bio = *(mp_bh->master_bio);
			...
			bio_set_dev(bio,
				    conf->multipaths[mp_bh->path].rdev->bdev);
...
static sector_t multipath_size(struct mddev *mddev, sector_t sectors,
			       int raid_disks)
{
...
static int multipath_run(struct mddev *mddev)
{
	...
	conf->multipaths = kcalloc(mddev->raid_disks,
				   sizeof(struct multipath_info), GFP_KERNEL);
	if (!conf->multipaths)
		goto out_free_conf;
	...
	rdev_for_each(rdev, mddev)
	{
		...
		if (disk_idx < 0 || disk_idx >= mddev->raid_disks)
			continue;
